...
int main(int argc, char *argv[])
{
    ...
    rwlockArray = malloc(array_size * sizeof(pthread_rwlock_t));
...
void *Operate(void *args)
{
    ...
    if (rqst.is_read == 0)
    {
        pthread_rwlock_wrlock(&(rwlockArray[rqst.pos]));
        setContent(&rqst.msg, rqst.pos, (char **)theArray);
        pthread_rwlock_unlock(&(rwlockArray[rqst.pos]));
    ...
    else
    {
        ...
        pthread_rwlock_rdlock(&rwlockArray[rqst.pos]);
